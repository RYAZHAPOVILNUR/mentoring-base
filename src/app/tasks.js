...
function getIdUsername() {
    return data.map((item) => console.log(item.id, item.username)); 
}

//2. Получить из данных выше список всех эмейлов

function getMail() {
    return data.map((item) => console.log(item.email));
}

//3. Вывести пользователя с id === 9

function getNine() {
    return data.find(item => item.id === 9); 
}

//4. Вывести всех пользователей из города 'Lebsackbury'

function getCity() {
    return data.find(item => item.city = 'Lebsackbury'); 
}
...
function getIncludeWord() {
    return data.map(item => item.website.includes('.com')); 
}

//9. Вывести все username которые длиннее 12 символов

function usernameLength() {
    return data.map((item) => item.length > 12); 
}

//10. Привести список всех username где каждый будет в нижнем регистре 

function usernameLowCase() {
    return data.map((item) => item.username.toLowerCase()); 
}
